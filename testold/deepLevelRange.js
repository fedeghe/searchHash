...
describe('Search deeper with level range', () => {
	describe('search for a value in a branch with more results gives the expected result', () => {
		...
		exps.forEach((exp) => {
			...
			it(`should find ${search1.length} elements with options ${JSON.stringify(opts1)}`, () => {
				assert.strictEqual(search1.length, exp.expected);
			});

			const opts2 = Object.assign(exp.opts, {}, { limit: 1 }),
				search2 = sh.forValue(objs, /^oNe$/i, opts2);
			it(`should find ${search2.length} elements with options ${JSON.stringify(opts2)}`, () => {
				assert.strictEqual(search2.length, exp.expected > 0 ? 1 : 0);
			});
		});
	});
});
